...
export const StatsView: React.FC = () => {
    // Flatten days to count workouts
    const allDays = planData.flatMap(w => w.days);
    
    // Calculate basic stats (Mocked logic for demo purposes where data might be sparse)
    const completedWorkouts = allDays.filter(d => d.isCompleted).length;
    // Assume a completed day averages 4km for this demo calculation
    const totalKmRun = completedWorkouts * 4; 
    
    // Mock data for the graph to look good immediately
    const weeklyVolume = [
        { week: 1, km: 25, status: 'completed' },
        { week: 2, km: 28, status: 'active' },
        { week: 3, km: 32, status: 'future' },
        { week: 4, km: 20, status: 'future' },
        { week: 5, km: 5, status: 'future' },
    ];
        ...
        <div className="flex flex-col p-6 gap-6 pb-32 animate-[fadeIn_0.5s]">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-5 rounded-3xl shadow-soft border border-slate-100 flex flex-col gap-2 relative overflow-hidden group">
                    ...
                    <span className="text-3xl font-display font-bold text-text-main">{completedWorkouts}</span>
                    <span className="text-xs text-text-light font-bold uppercase tracking-wider">Workouts Done</span>
                </div>
                <div className="bg-white p-5 rounded-3xl shadow-soft border border-slate-100 flex flex-col gap-2 relative overflow-hidden group">
                    <div className="absolute -right-4 -top-4 size-16 bg-secondary/5 rounded-full blur-xl group-hover:bg-secondary/10 transition-colors"></div>
                    <div className="size-10 rounded-full bg-secondary/10 flex items-center justify-center text-secondary mb-1">
                        <span className="material-symbols-outlined">local_fire_department</span>
                    </div>
                    <span className="text-3xl font-display font-bold text-text-main">3</span>
                    <span className="text-xs text-text-light font-bold uppercase tracking-wider">Day Streak</span>
                </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-3xl shadow-soft border border-slate-100">
                <div className="flex justify-between items-end mb-6">
                    ...
                    <span className="text-2xl font-bold text-primary-dark">28<span className="text-sm text-text-light font-normal ml-1">km this week</span></span>
                </div>
                             ...
                             <div className="w-full bg-slate-50 rounded-t-xl relative w-full max-w-[40px] flex items-end overflow-hidden h-full">
                                <div 
                                    className={`w-full rounded-t-xl transition-all duration-1000 ease-out ${
                                        w.status === 'completed' ? 'bg-secondary opacity-60' : 
             ...
             <div className="bg-white p-4 rounded-3xl shadow-soft border border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-text-light text-sm">Keep going!</p>
                    </div>
                </div>
